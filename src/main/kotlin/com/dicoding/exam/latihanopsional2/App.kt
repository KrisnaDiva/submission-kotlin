...
fun minAndMax(number: Int): Int {
    when (number.toString().length) {
        in 2..9 -> {
            val min = number.toString().minOrNull().toString().toInt()
            val max = number.toString().maxOrNull().toString().toInt()
            return min + max
        }
        else -> throw IllegalArgumentException("Number length must be between 2 and 9")
    }
}
